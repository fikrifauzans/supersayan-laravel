...
use App\Repositories\V1\Eloquent\Transaction\TransactionsRepository;
...
class TransactionsController extends Controller
{
    ...
    protected $validator;
    protected $response;
    protected $relations = ['TransactionDetails', 'Customer'];

    public function __construct(TransactionsRepository $repository, CustomValidator $validator, JsonCustomResponse $response)
    {
    ...
    public function index(Request $request, $language)
    {
        ...
        if (Gate::denies('permission',  'transactions-index')) return $this->response->error($language, 'permission-denied');
    ...
    public function show($language, $id, Request $request)
    {
        ...
        if (Gate::denies('permission',  'transactions-show')) return $this->response->error($language, 'permission-denied');
    ...
    public function store(Request $request, $language)
    {
        ...
        if (Gate::denies('permission',  'transactions-store')) return $this->response->error($language, 'permission-denied');
        ...
        return $this->response->choseLang($language)->modeMessage('create', $this->repository->getTableName())->success($this->repository->updateOrCreate(null, $request->all(), $this->repository->calculateStock($request)));
    }

    public function update(Request $request,  $language, $id)
    {

        if (Gate::denies('permission',  'transactions-update')) return $this->response->error($language, 'permission-denied');

        // $validated = $this->validator->validate($request, $language, $this->repository->getTableName());

        // if ($validated->fails()) return response()->json(['errors' => $validated->errors()], 422);

        return $this->response->choseLang($language)->modeMessage('update', $this->repository->getTableName())->success($this->repository->updateOrCreate($id, $request->all(), $request->only('transaction_details')));
    }

    public function destroy($language, $id, Request $request)
    {

        if (Gate::denies('permission',  'transactions-destroy')) return $this->response->error($language, 'permission-denied');
    ...
    public function force($language, $id, Request $request)
    {
        ...
        if (Gate::denies('permission',  'transactions-force')) return $this->response->error($language, 'permission-denied');
    ...
    public function restore($language, $id, Request $request)
    {
        ...
        if (Gate::denies('permission',  'transactions-restore')) return $this->response->error($language, 'permission-denied');

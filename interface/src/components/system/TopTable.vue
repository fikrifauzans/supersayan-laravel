    ...
    <div class="absolute-top-right q-pa-md">
      <q-btn-dropdown color="dark" size="sm" rounded flat icon="settings" menu-anchor="top left">
        <div class="row q-pa-xs q-gutter-sm">
          ...
          <!-- EXPORT  -->
          <q-btn v-if="this.$Handle.checkPermission(Meta.module + '-csv')" color="dark" round flat size="sm" rounded icon="archive" no-caps
    ...
    <div class="row col-12 justify-between">
      <div class="row">
        <q-item class="q-pa-none text-dark" color="primary" v-ripple>
          <q-item-section>
            ...
            <q-item-label style="font-weight: 700; font-size: 14px; line-height: 32px">Tabel {{
              title ? title : Meta ?
              Meta.name : ""
            }}
            </q-item-label>
        ...
        <div class="q-ml-md">
          <q-btn v-if="trash && table && table.selected.length > 0 &&  this.$Handle.checkPermission(Meta.module + '-restore')"  icon="settings_backup_restore" size="sm" rounded round
            class="bg-grey q-mt-sm text-white q-mr-xs" @click="modalConfirm.restore = !modalConfirm.restore">
            <q-tooltip> Restore </q-tooltip>
          </q-btn>
          <!-- DELETE  -->
          <q-btn v-if="!Meta.hide_delete && table && table.selected.length > 0 &&  this.$Handle.checkPermission(Meta.module + '-destroy')" icon="delete" size="sm" rounded round
            class="bg-negative q-mt-sm text-white" @click="modalConfirm.delete = !modalConfirm.delete">
      ...
      <div class="col-12 row items-center justify-between q-mt-sm">
        <div class="row items-center">
          <!-- Add  -->
          <div>
            <q-btn v-if="!Meta.hide_add &&
                this.$Handle.checkPermission(Meta.module + '-store')
                " :label="$q.screen.lt.md ? null : `Tambah ${Meta.name}`" size="md" no-caps flat icon="add"
              class="q-py-sm text-white bg-primary" glossy @click="$emit('add')" />
          </div>
        </div>
        <div class="row items-center">
          <!-- SEARCH TABLE  -->

          ...
          <s-form class="q-px-sm" :title="'Add Menus'">
            <template v-slot:right>
              <div class="q-gutter-xs">
                <q-btn :color="menus.type == 'parent' ? 'orange-3' : 'orange'" label="parent" size="sm"
                  @click="this.menus.type = 'parent'" />
                <q-btn :color="menus.type == 'child' ? 'orange-3' : 'orange'" label="child" size="sm"
                  @click="this.menus.type = 'child'" />
              </div>
            </template>

            <div class="col-12 row" v-if="menus.type != 'parent'">
              <t-select label="Select Parent" v-model="menus.sort" col="10" :option="model.menu_details"
                :filterField="'menu.name'" :optionValue="
                  (val) => (val == null || val.menu == undefined ? '' : val.sort)
                " :optionLabel="
  (val) => (val == null || val.menu == undefined ? '' : val.menu.name)
" />
            </div>
            <div class="col-12 row">
              <t-select-api label="Menus" api="menus?s=" field="name" v-model="menus.value" multiple col="10" full />
              <div class="col-2 text-center">
                <q-btn color="primary" class="q-mt-xs" rounded round icon="add" size="sm" noCaps
                  @click="pushParent()" />
              </div>
            </div>
          </s-form>
